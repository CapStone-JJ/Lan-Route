...
playlistRouter.post("/", authenticateUser, async (req, res, next) => {
  ...
  if (playlistUrl.includes("youtube.com") || playlistUrl.includes("youtu.be")) {
    ...
    embedCode = `<iframe src="https://open.spotify.com/embed/playlist/${getPlaylistIdFromUrl(
      playlistUrl
...
playlistRouter.get("/playlists/:category", async (req, res, next) => {
  ...
  try {
    ...
    switch (category) {
      ...
      case "YouTube":
        playlists = await prisma.playlist.findMany({
          where: { url: { contains: "youtube.com" } },
          include: { owner: { select: { username: true } } },
        });
        break;
      case "AppleMusic":
        playlists = await prisma.playlist.findMany({
          where: { url: { contains: "music.apple.com" } },
          include: { owner: { select: { username: true } } },
        });
        break;
      case "Spotify":
    ...
    async (req, res, next) => {
      ...
      try {
        ...
        if (existingPlaylist) {
          return res
            .status(400)
            .json({ error: "Playlist already added to your profile" });
        }
        ...
        res.json({
          message: "Playlist added to your profile successfully",
          user: updatedUser,
        });
      } catch (error) {
